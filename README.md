...
| 🎨 双主题 | 支持 Light（默认白底网格）和 Dark（蓝紫渐变）主题 |
| 🌙 默认主题 | 默认使用 Dark 主题 |
...
- 深色背景 `#0f0f1a` 配淡白色网格
- 蓝紫色渐变背景遮罩
- **状态胶囊条**：蓝紫色渐变 `rgba(102, 126, 234, 0.9) → rgba(118, 75, 162, 0.9)`，带发光亮光效果
- **背景设置按钮**：蓝紫色渐变 `#667eea → #764ba2`
- **日历任务标记**：有任务的日子显示浅绿色方格
- **霓虹灯呼吸效果**：状态指示器带呼吸动画
...
- **主题**：支持 Light/Dark 双主题，默认 Dark 主题
...
7. **主题系统**：通过 CSS 类切换实现 Light/Dark 主题切换
8. **CSS 渐变效果**：使用 `linear-gradient` 实现蓝紫色渐变和发光效果
